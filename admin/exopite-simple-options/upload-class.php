...
if ( ! class_exists( 'Exopite_Simple_Options_Framework_Upload' ) ) {
	...
	class Exopite_Simple_Options_Framework_Upload {
		...
		public static function file_uploader_callback() {
			...
			if ( strtoupper( sanitize_key( $_POST['_method'] ) ) == 'DELETE' && isset( $_POST['spartan_gallery'] ) ) {

				/**
				 * Delete file on AJAX request with spartan_gallery
				 *
...
//				$protocol = ( isset( $_SERVER['SERVER_PROTOCOL'] ) ? $_SERVER['SERVER_PROTOCOL'] : 'HTTP/1.0' );
		...
		public static function file_delete() {
			...
			$args = array(
				...
				'meta_query'  => array(
					array(
						'key'     => 'spartan_gallery',
						'value'   => sanitize_text_field( $_POST['spartan_gallery'] ),
						'compare' => '=',
					)
				)
		...
		public static function file_uploader() {
			...
			if ( $movefile ) {
				...
				// Add spartan_gallery for possibility to delete with AJAX fine uploader
				update_post_meta( $attachment, 'spartan_gallery', sanitize_text_field( $_POST['spartan_gallery'] ) );
